...
| `:- use_module(M)` / `:- use_module(M, Imports).` | Import another module. Import items use the same forms as export items. The wrapper `library(Name)` is accepted as a synonym for a bare module name. All `use_module` directives must appear before the first non-import directive or rule in the file (YCHR-20007). The prelude is the one module that cannot be narrowed: it is imported implicitly and in full, so an import list on it is rejected (YCHR-20019). |
