...
| Code | Name | Meaning |
|------|------|---------|
| `YCHR-60001` | `InconsistentTypes` | Two types cannot be unified. Also used for runtime errors raised by the Haskell interpreter (e.g. arithmetic on non-numbers, calling a non-function, a guard not evaluating to a boolean). Dispatch failures come in two flavours, distinguished by the message: `no matching equation in M:f/N` when every equation was ruled out on values that are already instantiated, and `argument K of M:f/N is not sufficiently instantiated …` when a pattern test was reached with an unbound variable at the position it inspects, so no verdict was possible. The second flavour is one of the *instantiation* failures (see [Soft guard failure](language.md#soft-guard-failure)): inside a rule guard it is caught and the guard evaluates to false, so the rule delays until reactivation instead of erroring; everywhere else (rule bodies, `is`, top-level goals) it is a hard error like any other. |
