...
The goal wraps the encoded term in `term/1` so it is passed as data. Without
the quote, the argument would be *evaluated* — and `var("x")` in particular
would call the prelude's `var/1` predicate instead of naming a variable node
(see [the language reference](../reference/language.md) on the `term/1`
quoting form):

```haskell
typecheckGoal :: Expr -> Term
typecheckGoal e =
  CompoundTerm (Unqualified "typecheck")
