...
- **Keyed `'$call'` dispatch**: `genCallFunDispatch` (`Compile.hs`) builds `call_1` … `call_10` as *linear* scans of `If` branches — one per function in the whole program whose arity matches, plus one for each lifted lambda at the arity its source lambda declared (a closure can only be invoked there, so no dead branches are emitted). The function-reference half was partly optimized on 2026-09-22: every branch in a dispatcher used to re-test the shared `'/'` closure shape and re-extract and re-test the closure's arity, so a program with *n* same-arity functions paid *n* shape tests, *n* extractions and *n* arity tests per `'$call'`. Those tests are now hoisted into one guard around the whole function-reference block, and the closure's name argument is extracted once, so each branch is a single `BEqual` against a local. Measured by interleaving five `cabal bench` rounds against the parent commit, `typecheck/pairs_library` went from a 252 ms median to 185 ms (base range 240–262 ms, hoisted range 180–231 ms; the benchmark's run-to-run spread is documented as ±10–15%, so the direction is unambiguous but not the exact figure). The scan is still linear in the number of same-arity functions the program (including all imported libraries) defines, and every lambda a program adds still lengthens the chain for other call sites of its declared arity. The VM already has the shape of the asymptotic fix: the *evaluables dispatch table* keys `(functor, arity)` to a procedure name for `EvalIs`, and `call_N` could resolve a closure the same way. This is the largest single item in the type checker's own cost — it is what the ~80% the checker pays for using the stdlib's higher-order list functions actually buys.
