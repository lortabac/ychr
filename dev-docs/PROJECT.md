...
- A second, all-CHR type checker is being built alongside it under `typechecker2/`, with `src/YCHR/Internal/TypeCheck/{Encode,V2}.hs` as its whole Haskell side: `Encode` turns a desugared program into one ground term, `V2` tells it to the CHR program and decodes the diagnostics that come back. Nothing user-facing calls it yet — `test/YCHR/TypeCheckDiffTest.hs` runs both checkers over every golden program that compiles (i.e. every directory except the compilation-negative ones) and compares their diagnostics, in report mode by default (`YCHR_TC_DIFF=strict` makes a mismatch a failure). Rendering is shared between the two ("TypeCheck.Render"), so the messages stay identical. `typechecker2/tc2_conform.chr` walks every node of an encoded program so an encoder/declaration mismatch fails loudly; it goes away once the real walkers cover the AST. The CHR side is: `tc2_ast.chr` (the encoded AST's declarations), `tc2_diag.chr` (diagnostics, the accumulators, and per-unit warning suppression), `tc2_env.chr` (the declaration environment), `tc2_pure.chr` (the checks that need no solver — duplicate constructors, type-definition validation, constructor arity), `tc2_solver.chr` (a fork of `typechecker/typechecker.chr`, rule for rule, with a structured `ctx` in place of the driver's integer handle), `tc2_skel.chr` (slot skeletons and variable collection), `tc2_walk.chr` (the declaration facts, the rule walk, the function-equation walk, and the class-function overload search, which runs each attempt in a `run_chr_session` sub-session), and `tc2_main.chr` (the two entry points). Goal checking is still to come; on whole programs the two checkers now report identical diagnostics over every golden program that compiles.
