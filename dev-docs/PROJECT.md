...
- Optional static type checker in `src/YCHR/Internal/TypeCheck.hs` (driver) and `src/YCHR/Internal/TypeCheck/{Compiled,Error,Render,TH}.hs`. Implemented as a CHR program (`typechecker/typechecker.chr`); programs without type annotations are accepted unchanged. It reports errors and warnings; `--Werror` promotes the latter. See [`docs/reference/type-system.md`](../docs/reference/type-system.md).
- A second, all-CHR type checker is being built alongside it under `typechecker2/`, with `src/YCHR/Internal/TypeCheck/{Encode,V2}.hs` as its whole Haskell side: `Encode` turns a desugared program into one ground term, `V2` tells it to the CHR program and decodes the diagnostics that come back. Nothing user-facing calls it yet — `test/YCHR/TypeCheckDiffTest.hs` runs both checkers over every golden program that compiles (i.e. every directory except the compilation-negative ones) and compares their diagnostics, in report mode by default (`YCHR_TC_DIFF=strict` makes a mismatch a failure). It compares three things per directory — the program, its rule bodies taken as one goal list, and each `.goal` file resolved the way a real query is — and additionally asserts — as a Hedgehog property, which fails rather than merely reporting — that the two agree on programs from the type-soundness generator (`test/YCHR/TypeSoundness/Gen.hs`), which is where the polymorphism, rigid type variables and guard-derived evidence live. Rendering is shared between the two ("TypeCheck.Render"), so the messages stay identical. `typechecker2/tc2_conform.chr` walks every node of an encoded program so an encoder/declaration mismatch fails loudly; it goes away once the real walkers cover the AST. The CHR side is: `tc2_ast.chr` (the encoded AST's declarations), `tc2_diag.chr` (the diagnostic vocabulary — `error/3` and `warning/3` are declared over `error_code`/`warning_code` and `error_detail` rather than `any`, so `ychr check --Werror` on the checker's own source rejects a report site that invents a code; the code↔detail pairing is not statically checked and is enforced instead by `decodeError` — plus the accumulators and per-unit warning suppression, keyed on a `unit_id` that spells "outside any checking unit" as `no_unit` instead of a sentinel integer), `tc2_env.chr` (the declaration environment), `tc2_pure.chr` (the checks that need no solver — duplicate constructors, type-definition validation, constructor arity), `tc2_solver.chr` (a fork of `typechecker/typechecker.chr`, rule for rule, with a structured `ctx` in place of the driver's integer handle), `tc2_skel.chr` (slot skeletons and variable collection), `tc2_walk.chr` (the declaration facts, the rule walk, the function-equation walk, the class-function overload search — which runs each attempt in a `run_chr_session` sub-session — and the goal walk), and `tc2_main.chr` (the two entry points). The two checkers now report identical diagnostics on every golden program that compiles, on every golden `.goal` file that resolves, and on generated programs; what is left is performance and the switchover. `tc2_env`, `tc2_pure`, `tc2_skel` and `tc2_walk` express their list traversals with `library(lists)` higher-order functions (`maplist`, `concat_map`, `foldl`, `filter`, `sort_by`, …) instead of hand-rolled recursion; `tc2_solver` is excluded because it is a rule-for-rule fork, and `tc2_conform` because it is temporary. That is a deliberate readability-over-speed choice, taken with the cost measured: it is worth about +80% of the checker's runtime over the golden corpus (13.4 s → 24.4 s for programs plus goal lists), spread over about 55 call sites at roughly 2–3x per list element, with no hot spot to reclaim — the cost is closure dispatch through `'$call'`, not wrapper overhead. The declaration facts a session needs are built as data before being told, so the overload search can hand a whole declaration environment to a sub-session (`retell_decls`, which `copy_term`s it first so no variable is shared across the boundary) instead of re-deriving it from the AST on every attempt.
...
- **Keyed `'$call'` dispatch**: `genCallFunDispatch` (`Compile.hs`) builds `call_1` / `call_2` as a *linear* chain of `If` branches — one per function in the whole program whose arity matches, plus one per lifted lambda — each testing `BMatchTerm` and one or two `BEqual`s on the closure term. Every `'$call'` therefore costs a scan proportional to the number of same-arity functions the program (including all imported libraries) happens to define, and every lambda a program adds lengthens the chain for every other call site. The VM already has the shape of the fix: the *evaluables dispatch table* keys `(functor, arity)` to a procedure name for `EvalIs`, and `call_N` could resolve a closure the same way. This is the largest single item in the V2 type-checker's performance gap — it is what the ~80% cost of using the stdlib's higher-order list functions actually buys.
