...
--
-- The 'StdLib' is the parsed bundled library, an explicit input because
-- the @ychr@ library carries no compile-time embedding of its own. Build
-- it with 'YCHR.Internal.StdLib.parseStdLib' from the @libraries\/*.chr@
-- sources — the @ychr@ executable's embedding component
...
-- program's own diagnostics are not diluted by stdlib warnings. Either
-- way the prelude is seeded, so the 'StdLib' is required.
--
-- Warnings accompany a successful compile; see 'Warning'.
compileModules ::
  StdLib -> Bool -> [(FilePath, Text)] -> Either Error (CompiledProgram, [Warning])
compileModules (StdLib stdlib) includeStdlib inputs = do
...
-- input.
compileParsedModules ::
  StdLib -> Bool -> [Module] -> Either Error (CompiledProgram, [Warning])
compileParsedModules (StdLib stdlib) includeStdlib parsed = do
  let userLibrarySeeds =
        noAnnP "prelude"
          : [ AnnP n loc p
            | m <- parsed,
