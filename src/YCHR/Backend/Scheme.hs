...
-- A program-info binding named after the last segment is also
-- exported (see 'programInfoBindingName').
  ...
  let procs = vmp.program.procedures
   ...
   in T.unlines $
        [ ";; Generated by YCHR",
          ...
          "  " <> renderSExpr (exportClause infoName vmp),
...
-- see 'YCHR.Desugared.BodyTell'), @%init!@, and the program-info
-- binding.
exportClause :: Text -> VMProgram -> SExpr
exportClause infoName vmp =
...
-- | Build a dispatcher procedure carrying the program's tell-procedure
-- catalogue and its @%init!@ thunk. Shape:
--
-- > (define (NAME key)
-- >   (case key
-- >     ((init)  (%init!))
-- >     ((tells) (list (cons "MOD:NAME/ARITY" tell_MOD__NAME_ARITY) ...))
-- >     (else (error 'NAME "unknown program-info key" key))))
--
-- The @tells@ payload is a list of @(string . procedure)@ pairs:
--
-- * /Key (car)/: the friendly surface form @"module:name/arity"@. Raw
--   user-source module and constraint names, no 'encodeText' applied —
--   this is what the user types at @(query s "..." ...)@, so it must
--   match their source-level intent (including any Unicode).
--
-- * /Value (cdr)/: the compiler-generated tell-procedure identifier,
--   which /is/ mangled via 'encodeText' and the @__@ separator (see
--   'tellProcName'). The cdr is a Scheme identifier, lexically bound
--   in this library; the user never types it.
--
-- 'YCHR.Backend.Scheme' is the authoritative producer of the mangling,
-- so the Scheme side never re-derives it.
...
programInfoSExpr infoName vmp =
  let procNames = Set.fromList [n.unName | p <- vmp.program.procedures, let n = p.name]
   ...
   in SList
        [ SAtom "define",
          SList [SAtom infoName, SAtom "key"],
          SList
            [ SAtom "case",
              SAtom "key",
              SList [SList [SAtom "init"], SList [SAtom "%init!"]],
              SList
                [ SList [SAtom "tells"],
                  SList (SAtom "list" : tellEntries)
                ],
              SList
                [ SAtom "else",
                  SList
                    [ SAtom "error",
                      SList [SAtom "quote", SAtom infoName],
                      SString "unknown program-info key",
                      SAtom "key"
                    ]
                ]
            ]
